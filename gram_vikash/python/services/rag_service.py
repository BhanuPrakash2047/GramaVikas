...
class RAGService:
    ...
    def diagnose_with_image(
        ...
        # 4. determine source & confidence from RAG if no image ─────
        rag_docs = [{"document": doc, "score": score} for doc, score in rag_results]
        source = "rag" if rag_results else "llm"

        # When no image was provided, use top RAG similarity score as confidence
        if not image_bytes and rag_results:
            confidence = float(rag_results[0][1])  # top hit similarity score
            logger.info("Using RAG similarity as confidence: %.3f", confidence)
        ...
        if language != "en":
            ...
            symptoms_matched = self.groq_service.translate_list(symptoms_matched, "en", language)
            management_advice = self.groq_service.translate_dict(management_advice, "en", language)
