...
class GroqService:
    ...
    def generate_diagnosis(
        ...
        system_prompt = (
            ...
            f"- Respond ONLY in {target_lang} – do not mix languages"
        )
        ...
        user_prompt = (
            ...
            f"general expert advice. Respond in {target_lang}."
        )
    ...
    def translate_list(self, items: List[str], source_lang: str, target_lang: str) -> List[str]:
        ...
        try:
            combined = " | ".join(items)
            translated = self.translate(combined, source_lang, target_lang)
            return translated.split(" | ")
        except Exception as exc:
            logger.error("List translation error: %s", exc)
            return items

    def translate_dict(self, data: Dict, source_lang: str, target_lang: str) -> Dict:
        """Translate all string values in a dictionary recursively."""

        if not data or source_lang == target_lang:
            return data
        ...
        try:
            for key, value in data.items():
                if isinstance(value, str):
                    translated[key] = self.translate(value, source_lang, target_lang)
                elif isinstance(value, list):
                    translated[key] = self.translate_list(
            ...
            logger.error("Dict translation error: %s", exc)
            return data
