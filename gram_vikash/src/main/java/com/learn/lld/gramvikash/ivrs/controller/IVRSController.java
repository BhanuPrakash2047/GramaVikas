...
public class IVRSController {
    ...
    /**
     * Emergency-voice webhook – receives transcribed speech for emergency reporting.
     * The speech is classified by GPT and dispatched via the emergency service.
     */
    @PostMapping(value = "/process-emergency", produces = MediaType.APPLICATION_XML_VALUE)
    public ResponseEntity<String> handleEmergency(
            @RequestParam("callSid") String callSid,
            @RequestParam("SpeechResult") String speechResult
    ) {
        log.info("IVRS emergency: CallSid={}, Speech={}", callSid, speechResult);
        return ResponseEntity.ok(ivrsService.handleEmergencyGathered(callSid, speechResult));
    }
}
