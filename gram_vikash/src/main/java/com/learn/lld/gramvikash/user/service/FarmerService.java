...
public class FarmerService {
    ...
    public void updateLanguage(Long farmerId, com.learn.lld.gramvikash.user.enums.Language language) {
        Farmer farmer = farmerRepository.findById(farmerId)
                .orElseThrow(() -> new RuntimeException("Farmer not found with id: " + farmerId));
    ...
    public FarmerProfileResponse updateProfile(Long farmerId, UpdateProfileRequest request) {
        Farmer farmer = farmerRepository.findById(farmerId)
                .orElseThrow(() -> new RuntimeException("Farmer not found with id: " + farmerId));
        ...
        if (request.getStateId() != null) {
            State state = entityManager.find(State.class, request.getStateId());
            if (state == null) throw new RuntimeException("State not found");
            farmer.setState(state);
        }
        if (request.getDistrictId() != null) {
            District district = entityManager.find(District.class, request.getDistrictId());
            if (district == null) throw new RuntimeException("District not found");
            farmer.setDistrict(district);
        }
        if (request.getMandalId() != null) {
            Mandal mandal = entityManager.find(Mandal.class, request.getMandalId());
            if (mandal == null) throw new RuntimeException("Mandal not found");
            farmer.setMandal(mandal);
        }

        Farmer saved = farmerRepository.save(farmer);

        Integer age = saved.getDob() != null
                ? Period.between(saved.getDob(), LocalDate.now()).getYears()
                : null;
        ...
        return FarmerProfileResponse.builder()
                ...
                .stateName(saved.getState().getName())
                .districtName(saved.getDistrict().getName())
                .mandalName(saved.getMandal().getName())
